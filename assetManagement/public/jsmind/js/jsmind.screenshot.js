...
resize: function () {
    if (this._inited) {
      // 获取当前视图的宽度和高度
      const currentWidth = this.jm.view.size.w;
      const currentHeight = this.jm.view.size.h;
  
      // 计算缩放比例
      const zoomScale = this.jm.getZoomScale(); // 这里的 jm.getZoomScale() 函数用于获取当前缩放比例
      ...
      // 设置新的宽度和高度
      this.canvas_elem.width = newWidth;
      this.canvas_elem.height = newHeight;
    }
  },
        clean:function(){
